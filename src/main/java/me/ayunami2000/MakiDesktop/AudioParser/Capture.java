...
package me.ayunami2000.MakiDesktop.AudioParser;

import me.ayunami2000.MakiDesktop.AudioCapture;
import me.ayunami2000.MakiDesktop.AudioParser.Analysis.Analysis;
import me.ayunami2000.MakiDesktop.MakiDesktop;
import me.ayunami2000.MakiDesktop.PitchDetection;
...
public class Capture implements Runnable{
	...
	public void run() {
		...
		if(MakiDesktop.audioDevice.equals(""))return;
			try{
				Mixer.Info[] mixerInfos = AudioSystem.getMixerInfo();
				Mixer mixer = null;
				for (int i = 0; i < mixerInfos.length; i++) {
					if (mixerInfos[i].getName().trim().equals(MakiDesktop.audioDevice)) {
				...
				if(mixer==null){
					for (int i = 0; i < mixerInfos.length; i++) {
						try {
							int index = Integer.parseInt(MakiDesktop.audioDevice);
							if (index==i) {
								mixer = AudioSystem.getMixer(mixerInfos[i]);
								break;
							}
				...
				if(mixer==null){
					System.out.println("Error: Audio device not found!");
					return;
				}
					...
					while (mainProgram.continueCapturing) {
						...
						if (count > 0) {
							...
							if (bitSelection ==2){
								...
								AudioCapture.pitchDetect(analysis);
