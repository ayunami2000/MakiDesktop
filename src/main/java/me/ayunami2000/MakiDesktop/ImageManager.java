...
public class ImageManager implements Listener {
     ...
     */
    public void init() {
        Bukkit.getServer().getPluginManager().registerEvents(this, MakiDesktop.getPlugin(MakiDesktop.class));
        loadImages();
    }
    ...
    public void onMapInitEvent(MapInitializeEvent event) {
        if (hasImage(event.getMap().getId())) {
            ...
            MakiDesktop.screens.add(new ScreenPart(view.getId(), getImage(view.getId())));
        }
    }
    ...
    static class CustomFile {
        ...
        private final MakiDesktop plugin = MakiDesktop.getPlugin(MakiDesktop.class);
